...
async function main() {
	try {
		...
		const useZipPackagesInput: string = core.getInput("USE_ZIP_PACKAGES", { required: false });
		...
		if (createRelease) { // Build, pack and release
			...
			for (let i = 0; i < projects.length; i++) {
				...
				await exec(`.\\dotnet-octo push --package=${project}\\artifacts\\${project}.${version}.zip --server=${octopusUrl} --apiKey=${octopusApiKey}`);
				}
				else {
